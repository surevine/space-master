...
# Note that this patch _DOESN'T_ use the generic patch script - no reason except that it pre-dates it!
...
then
    echo "    It seems as though Alfresco is already shut down, which is unexpected.  Would you like to proceed with the install anyway, without trying to shut alfresco down again?  Please select a numbered option"
    select proceed in "Proceed" "Abort"; do
        case $proceed in
            "Proceed" ) break;;
            "Abort" )   echo "Aborting..."; exit 2;;
        esac
    done
fi
...
echo "Applying patch"
cp -f *jar $ALFRESCO_HOME/tomcat/webapps/alfresco/WEB-INF/lib
echo "Patch Applied"
